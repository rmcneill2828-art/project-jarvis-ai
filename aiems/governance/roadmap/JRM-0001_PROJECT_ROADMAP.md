...
| **6** | Voice/Vision | **EBG-0112 registered at ESR-0039 closure** (unblocked once Phase 1 landed); **Increment A (speech output only) delivered at ESR-0040** per [[EIP-ESR0040-001_VOICE_PHASE6_INCREMENT_A_SPEECH_OUTPUT_SCOPE|EIP-ESR0040-001]] - `GuardianRuntime.speak()` synthesises Guardian's response text via a new Sentinel-gated, self-hosted Piper local-TTS provider (no API key, no recurring cost). Speech input (Increment B) and Vision (Increment C) remain not started, deliberately deferred: they introduce new data collection about people physically present, engaging GAM-0001 Section 8.1's Household Role Model in a way output-only voice does not, and each warrants its own dedicated future session. EBG-0081 Question 1's shared animation scheduler (Section 7.4) remains available for this phase's future animated UI surfaces (e.g. a voice waveform) once a UXP increment adds one. |
