...
Phase 2 (EBG-0021) **delivered at ESR-0041 WP1**, moved to Section 7.2 below. No item currently occupies this slot - Phase 3 (Action faculty implementation) is the next item in the numbered dependency chain but has no backlog item authorising its build yet (see Section 7.3 Phase 3).
...
| **3** | Action faculty (implementation) | **No backlog item yet builds this either** - Phase 2 defines the constraint (delivered above) but does not itself authorise building the Action faculty. Depends on Phase 1 (something must decide what to act on, delivered) and Phase 2 (the constraint it must obey, now delivered). STD-0006 (EBG-0065, see Section 7.4) should land before this phase ships, not after. |
