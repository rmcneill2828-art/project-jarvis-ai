...
| Primary Branch | main |
| Current Mode | [[ESR-0045_ENGINEERING_SESSION_REPORT|ESR-0045]] is the latest **closed** session. [[ESR-0046_ENGINEERING_SESSION_REPORT|ESR-0046]] is **open** (31 July 2026), continuing directly from ESR-0045's closure. **WP1** (Complete): resolved [[EBR-0001_ENGINEERING_BACKLOG_REGISTER|EBR-0001]] EBG-0116 (User Identity and Profile Foundation, the higher-leverage of [[LGB-0001_LAUNCH_GAP_BACKLOG|LGB-0001]]'s two Must-Ship items) per [[EIP-ESR0046-001_USER_IDENTITY_AND_PROFILE_FOUNDATION|EIP-ESR0046-001]]: new `jarvis/identity/` module (SQLite-backed `ProfileStore`/`ProfileService`, mirroring `jarvis/memory/`'s proven pattern), four new JSON-RPC methods (`profile.list`/`create`/`select`/`active`) and matching Tauri commands, and a real UXP profile picker replacing the previously static "Robert / Signed in locally" placeholder. Deliberately excludes credentialed authentication, memory scoping by profile, and enforcement of GAM-0001 Section 8.1's role authority - disclosed, separately-tracked follow-on work. Codex design review: Pass, no blocking findings. Programme Sponsor approval verified via the real Sponsor Approval Service before implementation. Full detail in [[ESR-0046_ENGINEERING_SESSION_REPORT|ESR-0046]]. |
| Current Repository Baseline | [[RBL-0027_REPOSITORY_BASELINE|RBL-0027]], accepted at ESR-0044 WP3 (superseding RBL-0026 - real Voice-faculty product wiring warranted a new baseline), retained at ESR-0045 WP7 (architecture/policy/documentation-only session, no code changed). ESR-0046 WP1 changed `jarvis/`, `src-tauri/` and `src/` - a genuine live product-capability change; baseline determination for this session remains open pending WP6/WP7. |
| Current Product Capability Baseline | [[PCB-0001_PRODUCT_CAPABILITY_BASELINE|PCB-0001]] v2.3, refreshed at ESR-0045 WP3 to reflect ESR-0040's Voice faculty delivery (speech output, self-hosted Piper), ESR-0041's Local Agent Permission Boundary, ESR-0043's persona refinement and ESR-0044's live-UXP Voice wiring - also corrected a further staleness the triggering review itself missed (Provider abstraction framework row/constraint still claimed no external provider was wired into the default runtime path, despite EBG-0070/ESR-0022 having wired one). Not yet refreshed for ESR-0046's User Identity and Profile Foundation delivery - noted as an observation, not actioned within EIP-ESR0046-001's approved scope. [[JARVIS_CAPABILITY_READINESS_MATRIX|JARVIS Capability Readiness Matrix]] was refreshed to v2.2 at ESR-0028 WP2/WP4; not yet refreshed for ESR-0040/0041/0043/0044/0046. |
...
| Current Phase | [[ESR-0046_ENGINEERING_SESSION_REPORT|ESR-0046]] open (31 July 2026) / WP1 delivered under the permanent [[EE-0001_INDEPENDENT_AI_PEER_REVIEW_TRIAL|EE-0001]] Section 7 appointment (Claude Engineering Implementer, ChatGPT/Codex Engineering Reviewer, Programme Sponsor gating). |
| Current Workflow | AIEMS Engineering Workflow v3 with Engineering Ecosystem Synchronisation working practice. ESR-0046 WP1 completed the standing draft/review/approval/implementation/commit/post-commit-review template entirely through direct `codex exec -s read-only` invocation and the deployed Sponsor Approval Service - confirmed repeatable across a twenty-first consecutive session (ESR-0026 through ESR-0046). |
| Current Engineering Objective | [[ESR-0046_ENGINEERING_SESSION_REPORT|ESR-0046]] open: WP1 resolved EBG-0116 (User Identity and Profile Foundation). See [[ESR-0046_ENGINEERING_SESSION_REPORT|ESR-0046]] for full detail and next-WP decision. |
