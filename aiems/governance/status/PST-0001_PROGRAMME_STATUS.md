...
| Primary Branch | main |
| Current Mode | [[ESR-0047_ENGINEERING_SESSION_REPORT|ESR-0047]] is the latest **closed** session (4 August 2026). No session is currently open. ESR-0047 delivered four Work Packages plus a WP6-caught fix round: **WP1** Documentation Debt Discipline (PBK-0001/COC-0001 stale RBL-0021 references corrected to RBL-0028); **WP2** investigated EBG-0118 (Codex CLI tooling stall) - inconclusive, remains open; **WP3** resolved [[EBR-0001_ENGINEERING_BACKLOG_REGISTER|EBR-0001]] EBG-0117 (Voice Faculty Increment B, Speech Input, the last of [[LGB-0001_LAUNCH_GAP_BACKLOG|LGB-0001]]'s two Must-Ship items) per [[EIP-ESR0047-001_VOICE_PHASE6_INCREMENT_B_SPEECH_INPUT_SCOPE|EIP-ESR0047-001]]: new self-hosted `faster-whisper` transcription provider, `GuardianRuntime.transcribe()`, `guardian.transcribe` RPC method, push-to-talk mic button; **WP4** a Repository Engineering Health Review (advisory only). Session-wide **WP6** (Independent Repository Verification): first pass **Fail** - Codex found the mic button activated the microphone before confirming transcription was configured, a real privacy-gating defect against the approved EIP; fixed directly (`GuardianRuntime.transcription_available`, new `platform.status` field, conditional rendering); re-reviewed **Pass, no findings**. Session-wide **WP7**: **Establish** [[RBL-0029_REPOSITORY_BASELINE|RBL-0029]] - a genuine, live-verified product-capability change. Full detail in [[ESR-0047_ENGINEERING_SESSION_REPORT|ESR-0047]]. |
| Current Repository Baseline | [[RBL-0029_REPOSITORY_BASELINE|RBL-0029]], accepted at ESR-0047 WP7 (superseding RBL-0028 - real Voice Faculty Increment B, Speech Input, product delivery). |
| Current Product Capability Baseline | [[PCB-0001_PRODUCT_CAPABILITY_BASELINE|PCB-0001]] v2.3, refreshed at ESR-0045 WP3. Not yet refreshed for ESR-0046's User Identity and ESR-0047's Voice Increment B deliveries - flagged as a documentation-staleness finding at ESR-0047 WP4, a candidate first item for a future session. [[JARVIS_CAPABILITY_READINESS_MATRIX|JARVIS Capability Readiness Matrix]] was refreshed to v2.2 at ESR-0028 WP2/WP4; not yet refreshed for ESR-0040 through ESR-0047 - also flagged at ESR-0047 WP4. |
...
| Current Phase | No session currently open. [[ESR-0047_ENGINEERING_SESSION_REPORT|ESR-0047]] closed (4 August 2026) / four Work Packages delivered under the permanent [[EE-0001_INDEPENDENT_AI_PEER_REVIEW_TRIAL|EE-0001]] Section 7 appointment (Claude Engineering Implementer, ChatGPT/Codex Engineering Reviewer, Programme Sponsor gating). |
| Current Workflow | AIEMS Engineering Workflow v3 with Engineering Ecosystem Synchronisation working practice. ESR-0047's WP2 investigated the EBG-0118 Codex CLI tooling stall first disclosed at ESR-0046; the specific 30+ minute silent hang did not reproduce, but a related, previously-known `CreateProcessAsUserW` sandbox spawn failure did, and was addressed by removing `[windows] sandbox = "elevated"` from `~/.codex/config.toml`. `codex exec` proved usable throughout the rest of the session, including a genuine session-wide WP6 that caught and required fixing a real defect - the standing template's post-commit review step is repeatable again, twenty-two sessions in (ESR-0026 through ESR-0047), with the one-session break at ESR-0046 disclosed rather than glossed over. |
| Current Engineering Objective | No session currently open. [[ESR-0047_ENGINEERING_SESSION_REPORT|ESR-0047]] closed: WP3 resolved EBG-0117 (Voice Faculty Increment B, Speech Input). RBL-0029 established. Both LGB-0001 Must-Ship items now resolved. See [[ESR-0047_ENGINEERING_SESSION_REPORT|ESR-0047]] for full detail. |
...
[[ESR-0047_ENGINEERING_SESSION_REPORT|ESR-0047]] is **Closed** (4 August 2026), run under the permanent Lead/Reviewer appointment: Claude as Engineering Implementer, Codex as Engineering Reviewer, Programme Sponsor gating every step.

**WP1** (Complete): Documentation Debt Discipline - PBK-0001 (1.34 to 1.35) and COC-0001 (1.16 to 1.17) both corrected, seven baselines stale on their RBL current-baseline reference (RBL-0021 to RBL-0028).

**WP2** (Complete): investigated [[EBR-0001_ENGINEERING_BACKLOG_REGISTER|EBR-0001]] EBG-0118 (Codex CLI tooling stall, first disclosed at ESR-0046). The disclosed 30+ minute silent hang did not reproduce across four live `codex exec` tests; a related, previously-known `CreateProcessAsUserW` sandbox spawn failure did, traced to `~/.codex/config.toml`'s `[windows] sandbox = "elevated"` setting - removed per Programme Sponsor direction, replacing the spawn crash with an immediate policy rejection rather than a hang. Net outcome disclosed as inconclusive on the actual stall; EBG-0118 remains open, Candidate Backlog.

**WP3** (Complete): resolved EBG-0117 (Voice Faculty Increment B: Speech Input) - the last of [[LGB-0001_LAUNCH_GAP_BACKLOG|LGB-0001]]'s two Must-Ship items - per [[EIP-ESR0047-001_VOICE_PHASE6_INCREMENT_B_SPEECH_INPUT_SCOPE|EIP-ESR0047-001]]. Push-to-talk microphone capture in the live UXP, transcribed via a new self-hosted `faster-whisper` provider (`sentinel/whisper_provider.py`), Sentinel-gated exactly like the existing speech-output path, populating the message composer for review before send - never auto-submitted. **Codex design-reviewed**: Pass with non-blocking findings, folded in before implementation. Programme Sponsor approval verified via the real Sponsor Approval Service. Full validation: `python -m pytest` 483 passed/1 skipped (was 453/1); Rust build/clippy/fmt/test clean; frontend build clean; Playwright 11/11 passed. Live smoke check: Guardian's own Piper-synthesized speech for a real sentence was transcribed back by Guardian's own Whisper path via the real `guardian.transcribe` RPC - exact text match.

**WP4** (Complete): Repository Engineering Health Review, advisory only. All 31 open EBR-0001 backlog items confirmed valid. Six documentation-staleness findings disclosed (RSC-0001, PCB-0001, JARVIS_CAPABILITY_READINESS_MATRIX, README.md, PST-0001, JRM-0001 all stale re: EBG-0116/EBG-0117 closure; EBR-0001's own Section 5A snapshot internally inconsistent). Handover recommends a batched Documentation Debt sync as the next session's WP1, then scoping EBG-0042 (Agent Framework Architecture). JARVIS Development Readiness Assessment: AIEMS sufficiently mature for full JARVIS Engineering.

**Session-wide WP6** (Independent Repository Verification): covering `f36a465..5c02d3d` (5 commits). **First pass: Fail** - Codex found a real defect: the mic button in `src/App.jsx` always rendered and activated `getUserMedia`/`MediaRecorder` before knowing whether transcription was configured, contradicting EIP-ESR0047-001 Section 5.5 item 12's explicit design. Fixed directly in the same WP6 pass: `GuardianRuntime.transcription_available` (new property) and a new `platform.status` `transcriptionAvailable` field let the frontend learn availability before rendering anything; the mic button now conditionally renders only when true. **Second pass: Pass, no findings** - Codex confirmed the fix genuinely closes the gap, the fix commit carries no unrelated changes, and the new/modified tests genuinely exercise the gating. Disclosed honestly as WP6 working as designed, not a review that rubber-stamped either pass.

**Session-wide WP7** (Repository Baseline Determination): **Establish** [[RBL-0029_REPOSITORY_BASELINE|RBL-0029]], superseding RBL-0028 - Programme Sponsor decision, matching the standing convention that a genuine live product-capability change warrants a new baseline. Full detail in [[ESR-0047_ENGINEERING_SESSION_REPORT|ESR-0047]].

---

## Prior Session: ESR-0046

[[ESR-0046_ENGINEERING_SESSION_REPORT|ESR-0046]] is **Closed** (31 July 2026). One Work Package: **WP1** resolved EBG-0116 (User Identity and Profile Foundation) per [[EIP-ESR0046-001_USER_IDENTITY_AND_PROFILE_FOUNDATION|EIP-ESR0046-001]] - new `jarvis/identity/` module, four new `profile.*` RPC methods, real UXP profile picker. Codex design review Pass; the standing post-commit re-review could not complete (EBG-0118, disclosed this session as inconclusively investigated at ESR-0047 WP2) - performed directly instead. Session-wide WP6 performed directly, WP7 Establish [[RBL-0028_REPOSITORY_BASELINE|RBL-0028]] (superseded by RBL-0029 at ESR-0047). Programme Sponsor closed the session after one WP, concerned about the Codex tooling issue. Full detail in [[ESR-0046_ENGINEERING_SESSION_REPORT|ESR-0046]].

---

PST-0001 records ESR-0047 as closed, with no session currently open. It does not itself approve implementation outside separately authorised engineering work. Full historical detail remains in each session's own report and [[REG-0001_CONTROLLED_ARTEFACT_REGISTER|REG-0001]]'s version history.
...
| JARVIS Product Capability Baseline | Accepted, Stale | Foundation | [[PCB-0001_PRODUCT_CAPABILITY_BASELINE|PCB-0001]] v2.3 - refreshed at ESR-0045 WP3 for ESR-0040/0041/0043/0044 (Voice faculty, Local Agent Permission Boundary, persona refinement, live-UXP Voice wiring). Now stale again for ESR-0046 (User Identity) and ESR-0047 (Voice Increment B) - disclosed at ESR-0047 WP4's Repository Engineering Health Review, recommended for the next session's Documentation Debt sync. |
| JARVIS Capability Maturity | Maintained, Current | Foundation | [[JARVIS_CAPABILITY_READINESS_MATRIX|JARVIS Capability Readiness Matrix]] refreshed to v2.2 at ESR-0028 WP2/WP4 - no longer stale. Guardian, Sentinel, Platform Services, UXP, Knowledge and Provider Architecture shown as Implemented (Foundation); Memory shown as Partial (Personal tier implemented at ESR-0027 WP1, Session/Shared-Family tiers not yet built); Voice, Vision, Home Automation, Productivity, Multi-device and a JARVIS-internal Engineering Agent remain correctly Not Started. |
...
| Current Product Baseline | [[PCB-0001_PRODUCT_CAPABILITY_BASELINE|PCB-0001]] | v2.3 - refreshed at ESR-0045 WP3 for ESR-0040/0041/0043/0044 (Voice faculty, Local Agent Permission Boundary, persona refinement, live-UXP Voice wiring, and a corrected provider-wiring claim). Sibling [[JARVIS_CAPABILITY_READINESS_MATRIX|JARVIS Capability Readiness Matrix]] refreshed to v2.2 at ESR-0028 WP2/WP4, not yet refreshed for the same ESR-0040/0041/0043/0044 delivery. |
...
| Next Work Package Candidate | Not yet selected - remaining Section 5A themes (Theme 3 Guardian Action/Local Agent Boundary, Theme 6 AIEMS process tooling remainder, Theme 7's 7 still-open items, Theme 8 deferred research) remain open candidates for a future session's own WP0B, alongside EBG-0085 (Vite 8 upgrade, explicitly deferred this session). | Not decided by this status update - candidates only. |
...
| Repository Acceptance | Accepted through [[RBL-0023_REPOSITORY_BASELINE|RBL-0023]] |
...
| Product Capability Baseline | [[PCB-0001_PRODUCT_CAPABILITY_BASELINE|PCB-0001]] v2.3 (refreshed ESR-0045 WP3); sibling [[JARVIS_CAPABILITY_READINESS_MATRIX|JARVIS Capability Readiness Matrix]] v2.2 (ESR-0028 WP2/WP4) |
| Latest Repository Product Capability Assessment | [[RPCA-0001_REPOSITORY_PRODUCT_CAPABILITY_ASSESSMENT|RPCA-0001]] |
| Current Activity | See Section 3 (Current Mode) / Section 4A (Current Engineering Focus) - restructured at ESR-0031 (Documentation Debt Discipline) to reference the single authoritative source rather than restate it independently. Latest figures: `python scripts/validate_repository.py` 0 errors, 177 warnings; `python -m pytest` 382/382 passing (plus 1 correctly-skipped win32-conditional test). |
