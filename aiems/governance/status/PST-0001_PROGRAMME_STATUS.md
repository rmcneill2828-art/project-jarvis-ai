...
| Primary Branch | main |
| Current Mode | [[ESR-0036_ENGINEERING_SESSION_REPORT|ESR-0036]] is the latest **closed** session (26 July 2026). [[ESR-0037_ENGINEERING_SESSION_REPORT|ESR-0037]] is currently **open** (28 July 2026), opened directly from ESR-0036's closure, per PBK-0001 WP0B this row continues naming the latest closed session until ESR-0037 itself closes. **WP1** (In Progress): diagnose and fix [[EBR-0001_ENGINEERING_BACKLOG_REGISTER|EBR-0001]] EBG-0109 (live Guardian conversation path unreliable against the Ollama provider in the Tauri desktop shell), selected ahead of EBG-0108 (Guardian Cognitive Core Implementation) because EBG-0108's own entry names this exact reliability gap as a dependency. Full detail in [[ESR-0037_ENGINEERING_SESSION_REPORT|ESR-0037]]. |
...
| Current Phase | ESR-0037 open (28 July 2026); under the permanent [[EE-0001_INDEPENDENT_AI_PEER_REVIEW_TRIAL|EE-0001]] Section 7 appointment (Claude Engineering Implementer, ChatGPT/Codex Engineering Reviewer, Programme Sponsor gating). |
| Current Workflow | AIEMS Engineering Workflow v3 with Engineering Ecosystem Synchronisation working practice. ESR-0036 completed the standing draft/review/approval/implementation/commit/post-commit-review template entirely through `scripts/aiems_bridge.py` and the deployed Sponsor Approval Service - confirmed repeatable across an eleventh consecutive session (ESR-0026 through ESR-0036); ESR-0037 continues the same practice. |
| Current Engineering Objective | ESR-0037 WP1: diagnose and fix EBG-0109 (Ollama/Tauri conversation-path reliability). See [[ESR-0037_ENGINEERING_SESSION_REPORT|ESR-0037]] for full detail. |
...
[[ESR-0037_ENGINEERING_SESSION_REPORT|ESR-0037]] is **open** (28 July 2026), created at WP0B before WP1 began, correcting the process gap disclosed at ESR-0036 (report created retroactively during WP2). Run under the permanent Lead/Reviewer appointment: Claude as Engineering Implementer, Codex as Engineering Reviewer, Programme Sponsor gating every step.

**WP1** (In Progress): diagnose and fix [[EBR-0001_ENGINEERING_BACKLOG_REGISTER|EBR-0001]] EBG-0109 - the live Guardian conversation path is unreliable against the Ollama provider in the Tauri desktop shell, combining a reasoning-model timeout exposure (Finding 1: `qwen3.5:2b`'s default context/thinking behaviour routinely exceeds `OLLAMA_TIMEOUT_SECONDS`) with a distinct, more serious Tauri IPC-path defect (Finding 2: silent `LocalEchoProvider` fallback and indefinite hangs with no user-visible error, reproducible even with a fast non-reasoning model). Selected ahead of EBG-0108 (Guardian Cognitive Core Implementation) because EBG-0108's own entry names this exact reliability gap as a dependency - no open EBR-0001 item concerns documentation staleness, so PBK-0001's Documentation-Debt Priority discipline does not apply this session. Full detail in [[ESR-0037_ENGINEERING_SESSION_REPORT|ESR-0037]].
...
PST-0001 records ESR-0037 as the currently open session. It does not itself approve implementation outside separately authorised engineering work. Full historical detail remains in each session's own report and [[REG-0001_CONTROLLED_ARTEFACT_REGISTER|REG-0001]]'s version history.
