...
| Primary Branch | main |
| Current Mode | [[ESR-0040_ENGINEERING_SESSION_REPORT|ESR-0040]] is the latest **closed** session (29 July 2026). No session is currently open. **WP1** closed [[EBR-0001_ENGINEERING_BACKLOG_REGISTER|EBR-0001]] EBG-0112 Increment A: Guardian gains its first Voice faculty capability, speech output only, via a new Sentinel-gated, self-hosted Piper local-TTS provider - approved via [[EIP-ESR0040-001_VOICE_PHASE6_INCREMENT_A_SPEECH_OUTPUT_SCOPE|EIP-ESR0040-001]] v1.2 (revised mid-session from an initial ElevenLabs design after a live smoke check failed on a Free-plan API restriction; Codex design-reviewed twice, both Pass; Programme Sponsor approval verified via the real Sponsor Approval Service). Session-wide **WP2** (Independent Repository Verification: Pass, no findings) and **WP3** (Repository Baseline Determination: **Establish** - [[RBL-0025_REPOSITORY_BASELINE|RBL-0025]] accepted, superseding RBL-0024) closed the session. Full detail in [[ESR-0040_ENGINEERING_SESSION_REPORT|ESR-0040]]. |
| Current Repository Baseline | [[RBL-0025_REPOSITORY_BASELINE|RBL-0025]], accepted at ESR-0040 WP3, superseding RBL-0024 - WP1's real Guardian Voice faculty delivery, with new test coverage and live-verified audio output, warranted a new baseline. |
...
| Current Phase | ESR-0040 closed (29 July 2026); no session currently open; under the permanent [[EE-0001_INDEPENDENT_AI_PEER_REVIEW_TRIAL|EE-0001]] Section 7 appointment (Claude Engineering Implementer, ChatGPT/Codex Engineering Reviewer, Programme Sponsor gating). |
| Current Workflow | AIEMS Engineering Workflow v3 with Engineering Ecosystem Synchronisation working practice. ESR-0040 completed the standing draft/review/approval/implementation/commit/post-commit-review template entirely through `scripts/aiems_bridge.py` and the deployed Sponsor Approval Service - confirmed repeatable across a fifteenth consecutive session (ESR-0026 through ESR-0040), including a mid-session design revision and re-approval cycle. No session is currently open; the next session will select its own objective at WP0B. |
| Current Engineering Objective | None - no session currently open. ESR-0040's WP1 plus session-wide WP2/WP3 are all Complete; see [[ESR-0040_ENGINEERING_SESSION_REPORT|ESR-0040]] for full detail. |
...
[[ESR-0040_ENGINEERING_SESSION_REPORT|ESR-0040]] is **closed** (29 July 2026). **No session is currently open.** The fifteenth session run entirely through the AIEMS Exchange Bridge with no manual relay anywhere, run under the permanent Lead/Reviewer appointment: Claude as Engineering Implementer, Codex as Engineering Reviewer, Programme Sponsor gating every step.

**WP1** (Complete): closed [[EBR-0001_ENGINEERING_BACKLOG_REGISTER|EBR-0001]] EBG-0112 Increment A (Guardian Voice faculty, speech output only) - JRM-0001 Track B Phase 6's first increment, unblocked once Phase 1 (Guardian Cognitive Core) closed at ESR-0039. A new parallel Sentinel speech-synthesis contract (`sentinel/speech_providers.py`) and a Sentinel-gated voice interface (`jarvis/interfaces/voice.py`, `SpeechOutcome` envelope) back `GuardianRuntime.speak()`. **Mid-session design revision**: the initial [[EIP-ESR0040-001_VOICE_PHASE6_INCREMENT_A_SPEECH_OUTPUT_SCOPE|EIP-ESR0040-001]] v1.0 was implemented and Codex design-reviewed against ElevenLabs (cloud HTTP API), but its live smoke check failed with `HTTP 402` (Free-plan API restriction); the Programme Sponsor identified ElevenLabs as a new paid-cloud-vendor relationship with no repository precedent, against the project's self-hosted/no-discretionary-budget default, and directed a pivot to self-hosted Piper local TTS (`sentinel/piper_provider.py`, `piper-tts` - the project's first runtime dependency beyond `psutil`). The revision was separately Codex design-reviewed (v1.1/v1.2, both Pass) and Programme Sponsor-approved via the real Sponsor Approval Service (`submit-response`, not chat assertion) before re-implementation. Speech input and Vision remain explicitly out of scope, deferred to future increments engaging GAM-0001 Section 8.1's Household Role Model. 22 new tests; full suite 418 passed, 1 skipped (was 396/1). Live end-to-end validation confirmed a genuine 190,508-byte `audio/wav` payload via the real Piper adapter. [[AAM-0001_GUARDIAN_IDENTITY_AND_COGNITIVE_ARCHITECTURE|AAM-0001]] updated to record the Voice faculty, Phase 6 Increment A as implemented. Full detail in [[ESR-0040_ENGINEERING_SESSION_REPORT|ESR-0040]].

Session-wide **WP2** (Independent Repository Verification): **Pass, no findings** - Codex independently reviewed the pushed commit, confirmed diff scope matched approvals (including confirming the superseded ElevenLabs files are genuinely absent from the final tree), confirmed no `src/`, `src-tauri/`, `jarvis/memory/` or `.github/workflows/` file was touched. Session-wide **WP3** (Repository Baseline Determination): **Establish** - [[RBL-0025_REPOSITORY_BASELINE|RBL-0025]] accepted as the new current repository baseline, superseding [[RBL-0024_REPOSITORY_BASELINE|RBL-0024]].

418 Python tests pass plus 1 correctly-skipped test (was 396 plus 1); `validate_repository.py` (full mode) passes clean (192 warnings, up from 190 at session start - two new cross-document Section-reference false positives from this session's own new artefacts, consistent with the established disclosed false-positive category).

---

## Prior Session: ESR-0039

[[ESR-0039_ENGINEERING_SESSION_REPORT|ESR-0039]] is **Closed** (29 July 2026). The fourteenth session run entirely through the AIEMS Exchange Bridge with no manual relay anywhere. **WP1** closed EBG-0108 Phase 1 (Guardian Cognitive Core Implementation): a new `GuardianCognitiveCore` composes persona, retained Personal Memory content and bounded 6-exchange conversation history before each provider call, replacing the previous stateless single-turn pass-through. Session-wide WP2 Pass (no findings), WP3 Establish - [[RBL-0024_REPOSITORY_BASELINE|RBL-0024]] accepted, superseding RBL-0023. Full detail in [[ESR-0039_ENGINEERING_SESSION_REPORT|ESR-0039]].

---

PST-0001 records ESR-0040 as the latest closed session and no session currently open. It does not itself approve implementation outside separately authorised engineering work. Full historical detail remains in each session's own report and [[REG-0001_CONTROLLED_ARTEFACT_REGISTER|REG-0001]]'s version history.
