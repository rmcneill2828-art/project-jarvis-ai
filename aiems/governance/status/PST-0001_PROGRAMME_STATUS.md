...
| Primary Branch | main |
| Current Mode | [[ESR-0030_ENGINEERING_SESSION_REPORT|ESR-0030]] is the latest **closed** session (19 July 2026). **ESR-0031 is now open, in progress.** ESR-0030 was the fifth session run entirely through the AIEMS Exchange Bridge (`scripts/aiems_bridge.py`) with no manual relay anywhere: WP1 implemented the Sponsor Approval Service (ADR-0022) - `scripts/sponsor_approval_service.py`, `scripts/sponsor_client.py`, and the `scripts/aiems_bridge.py` diff deleting `cmd_sponsor_decision` entirely - and WP2 confirmed the Programme Sponsor's own Tailscale deployment of that service (real tailnet address, sponsor token held exclusively on a genuinely separate Android device), closing EBG-0084 in full. Session-wide WP3 (Pass, after three self-inflicted fix rounds on the handover document itself) and WP4 (Retain - RBL-0017 unchanged, no new baseline) both closed via the bridge. This was the first session where the approval step itself ran through a real deployed Sponsor Approval Service rather than a local file or bootstrap instance - `submit-response` was genuinely exercised throughout, including real cross-device Sponsor-originated approvals. 338 tests total (up from 295). Full detail in [[ESR-0030_ENGINEERING_SESSION_REPORT|ESR-0030]]. **ESR-0031 WP0** (repository synchronisation - corrected stale "jarvis/memory/ empty stub" claims across PST-0001/MDS-0001/PCB-0001 and one code-level instance in `jarvis/platform/shell.py`, across three fix rounds), **WP0B** (registered draft EIP-ESR0031-001, AIEMS Session-Opening Launcher), **WP1** (`scripts/session_launcher.py` implemented and reviewed clean, EBG-0097 Complete), **WP2B** (registered draft EIP-ESR0031-002, Streaming Notifications MVP - the session's paired product-moving objective, after Shared Family Memory was ruled out as premature per GAM-0001 Section 10's own explicit non-goal against household identity/authentication enforcement), **WP2** (heartbeat emitter, Rust reader-thread restructuring, React notification listener all implemented and live-smoke-tested via a real running Tauri session, EBG-0099 Complete), session-wide **WP3** (Independent Repository Verification: Pass, after one fix round on the handover's own self-referential HEAD-staleness) and session-wide **WP4** (Repository Baseline Acceptance: **new baseline RBL-0018 accepted**, superseding RBL-0017, both independent views converging that this session delivered a genuine live-verified UXP capability) are all complete. This session also directed a new PBK-0001 practice, Documentation Debt Discipline (whole-document staleness sweeps on edit; documentation-debt backlog gets first priority until cleared), following a live example found during this same closure - Section 8/9's duplicate "current state" rows had drifted stale for five sessions running while this Current Mode row was kept current throughout. 351 tests total (up from 338). |
...
ESR-0031 has all Work Packages complete, Engineering Session Report authorship pending (this session's report does not exist as a controlled artefact yet, per established practice - a new session report is not created until closure), run under the permanent Lead/Reviewer appointment: Claude as Engineering Implementer, Codex (ChatGPT) as Engineering Reviewer, Programme Sponsor gating every step. **Every Work Package this session was reviewed via a refined `codex exec -s read-only` pattern - Codex has zero write capability by construction, reporting findings as plain text that Claude relays into the bridge under explicit per-instance Sponsor approval - replacing an earlier approach that depended on a fragile Windows sandbox permission grant.**
...
| Next Required Activity | Author the ESR-0031 Engineering Session Report and formally close the session | All Work Packages complete including session-wide WP3 (Pass)/WP4 (RBL-0018 accepted); closure authorship is the only remaining step. |
| Next Work Package Candidate | Not yet determined - to be selected by the Programme Sponsor once ESR-0031 formally closes, informed by [[EBR-0001_ENGINEERING_BACKLOG_REGISTER|EBR-0001]]'s open High-priority items (including EBG-0098 and EBG-0100, both registered this session) and [[JRM-0001_PROJECT_ROADMAP|JRM-0001]]'s Near-term candidates. | Not decided by this status update. |
...
| Authoritative Backlog Source | [[EBR-0001_ENGINEERING_BACKLOG_REGISTER|EBR-0001]] | Future engineering priorities remain governed by the backlog register, sequenced via [[JRM-0001_PROJECT_ROADMAP|JRM-0001]]; EBG-0097/EBG-0099 marked Complete at ESR-0031, EBG-0098/EBG-0100 registered as Candidate Backlog. |
...
7. Review [[ESR-0030_ENGINEERING_SESSION_REPORT|ESR-0030]] (Current ESR tier, Closed) - the fifth session run entirely through `scripts/aiems_bridge.py` with no manual relay, delivering the Sponsor Approval Service (ADR-0022/EBG-0084) with genuine Sponsor-side Tailscale deployment - the first session where the approval step itself ran through a real deployed service rather than a local file. RBL-0017 retained, no new baseline. Also review [[ESR-0023_ENGINEERING_SESSION_REPORT|ESR-0023]] Sections 9.1/10.1/10.2 as a live example of a repository write-boundary deviation, its root cause, and how it was found and fixed - directly relevant to the bridge's role-locked-permissions design.
...
This guidance records the ESR-0030 closed state and RBL-0017's retention at ESR-0030 WP4 (unchanged since ESR-0029 WP9, superseding RBL-0016). PST-0001 does not create a new engineering session or approve implementation outside separately authorised engineering work.
