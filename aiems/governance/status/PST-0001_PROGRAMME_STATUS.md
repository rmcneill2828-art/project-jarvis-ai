...
**WP1** (Complete): content-refreshed [[PCB-0001_PRODUCT_CAPABILITY_BASELINE|PCB-0001]] and [[JARVIS_CAPABILITY_READINESS_MATRIX|JARVIS Capability Readiness Matrix]] for the Agent Framework Phase 3 capability delivered at [[ESR-0049_ENGINEERING_SESSION_REPORT|ESR-0049]] - both artefacts received only a baseline-pointer sync at ESR-0049 WP7, with capability content explicitly disclosed as deferred. A Whole-Document Staleness Sweep on Edit caught further PST-0001 drift beyond this WP's own scope (Repository Health block seven baselines stale) in the same pass.

**WP2** (Complete): resolved [[EBR-0001_ENGINEERING_BACKLOG_REGISTER|EBR-0001]] EBG-0120 (Agent Framework UXP Wiring) per [[EIP-ESR0050-001_AGENT_FRAMEWORK_UXP_WIRING|EIP-ESR0050-001]] - two new Tauri commands (`list_agents`/`invoke_agent`), a live `agent-framework` capability-sidebar row, a new `AgentFrameworkPanel` (list agents, Run button, real returned payload, denied/error outcomes shown inline), `App()` state/handlers mirroring `handleSpeak`'s pattern, and 4 new Playwright tests (13/13 passing). GIA's `gia-observability` agent is now invokable live through the UXP, closing the gap ESR-0049 WP2 itself disclosed as excluded. **Codex design-reviewed**: v0.1 Pass with 3 non-blocking findings, folded into v0.2. Programme Sponsor approval-to-implement verified via `submit-response`. **Post-commit review**: first attempt returned a false Fail, traced to imprecise wording in the review prompt itself (governance prose legitimately naming `GAM-0001`/`sentinel/policy.py` to disclose non-modification, misread by an over-literal claim) - not a defect in the committed code; corrected and re-run, **Pass, no findings**. Push likewise gated through the real Sponsor Approval Service. `GAM-0001` Section 8A's `LOCAL_AGENT_ACTION` remains completely untouched; `sentinel/policy.py` not modified.

**WP3** (Complete): resolved EBG-0047 (Sentinel Gate of Durin Architecture Specification) per [[EIP-ESR0050-002_CURRENT_ARCHITECTURE_REFRESH_AND_GATE_OF_DURIN_DETAIL|EIP-ESR0050-002]]. Investigation found `CURRENT_ARCHITECTURE.md`, not SAM-0001, was the item's real target - self-designated authoritative, already named EBG-0047 as its own extension point, but 34 sessions stale with one factual error (claimed `SimpleApprovalPolicy` remains the production default; `TrustTierPolicy` has been real since ESR-0024 WP1/EBG-0074). Registered as a controlled artefact for the first time (v1.0), broadly refreshed (Programme Sponsor's explicit scope-boundary choice over a narrower alternative), gained a new Sentinel Gate of Durin subsection. **Codex design-reviewed**: v0.1 Pass with 1 non-blocking finding, folded into v0.2. **Post-commit review**: Pass with 1 non-blocking finding (a pre-existing `sentinel/policy.py` docstring, out of this documentation-only package's scope). Documentation only, no code touched.

**Session-wide WP6** (Independent Repository Verification): genuine background `codex exec -s read-only` review of the full `0d614e0..a7eabb0` diff (WP1+WP2+WP3). Confirmed: exactly the expected 14 files changed; `GAM-0001` and `sentinel/policy.py` byte-identical/untouched; no backend Agent Framework code touched; the UXP wiring and `CURRENT_ARCHITECTURE.md` refresh both confirmed genuinely present against the real current file state; every "Completed" governance claim backed by real evidence; every REG-0001 version row matching. **Verdict: Pass, no findings.**

**Session-wide WP7** (Repository Baseline Determination): **Establish** [[RBL-0031_REPOSITORY_BASELINE|RBL-0031]], superseding [[RBL-0030_REPOSITORY_BASELINE|RBL-0030]] - WP2 delivered a genuine, live product-capability change (the Agent Framework moved from RPC/backend-reachable only to genuinely invokable through the live UXP), matching the threshold applied at RBL-0025/RBL-0027/RBL-0028/RBL-0029/RBL-0030 rather than the Retain threshold applied at architecture/documentation-only sessions such as ESR-0048.

---

## Prior Session: ESR-0049

[[ESR-0049_ENGINEERING_SESSION_REPORT|ESR-0049]] is **Closed** (5 August 2026). Two Work Packages: **WP1** Documentation Debt Discipline fix (COC-0001's stale RBL-0028 reference, found at this session's own WP0A); **WP2** resolved [[EBR-0001_ENGINEERING_BACKLOG_REGISTER|EBR-0001]] EBG-0119 (Agent Framework Phase 3: First Specialist Agent Implementation) per [[EIP-ESR0049-001_AGENT_FRAMEWORK_PHASE3_FIRST_SPECIALIST_AGENT|EIP-ESR0049-001]] - real code, not architecture-only: new `jarvis/agents/` module, `SentinelGatedAgentService` reusing the shared Sentinel gateway, `GuardianRuntime`/`build_default_runtime()` wiring, two new `guardian.agent.*` JSON-RPC methods, an honest `platform.status` correction. GIA's existing read-only observability wired as the first live `ROUTINE_INTERACTION` specialist agent - `GAM-0001` Section 8A's `LOCAL_AGENT_ACTION` hard `DENY` untouched. 27 new tests (512 passed/1 skipped, up from 485/1). Session-wide WP6 (genuine background Codex review of the full session diff) Pass, no findings. Session-wide WP7: Establish [[RBL-0030_REPOSITORY_BASELINE|RBL-0030]], superseding RBL-0029 - a genuine live product-capability change. Full detail in [[ESR-0049_ENGINEERING_SESSION_REPORT|ESR-0049]].
...
| JARVIS Product Capability Baseline | Accepted, Current | Foundation | [[PCB-0001_PRODUCT_CAPABILITY_BASELINE|PCB-0001]] v2.7 - baseline-pointer sync at ESR-0050 WP7 (RBL-0031); content-refreshed at WP1 for the Agent Framework Phase 3 capability delivered at ESR-0049. WP2's own Agent Framework UXP Wiring not yet reflected in content, flagged for a future sync. |
| JARVIS Capability Maturity | Maintained, Current | Foundation | [[JARVIS_CAPABILITY_READINESS_MATRIX|JARVIS Capability Readiness Matrix]] v2.6 (pointer-synced ESR-0050 WP7; content-refreshed WP1) - no longer stale. Guardian, Sentinel, Platform Services, UXP, Knowledge, Provider Architecture and the Agent Framework (GIA's read-only observability, now invokable live through the UXP, ESR-0049/ESR-0050) shown as Implemented (Foundation); Memory shown as Partial (Personal tier implemented at ESR-0027 WP1, Session/Shared-Family tiers not yet built); Intelligence, Identity and Voice all now Implemented (Phase 1/Foundation); Vision, Home Automation, Productivity and Multi-device remain correctly Not Started. |
...
| Current Product Baseline | [[PCB-0001_PRODUCT_CAPABILITY_BASELINE|PCB-0001]] | v2.7 - pointer-synced at ESR-0050 WP7 (RBL-0031); content-refreshed at WP1 for the Agent Framework Phase 3 capability delivered at ESR-0049. Sibling [[JARVIS_CAPABILITY_READINESS_MATRIX|JARVIS Capability Readiness Matrix]] v2.6, same pattern. |
...
| Repository Acceptance | Accepted through [[RBL-0031_REPOSITORY_BASELINE|RBL-0031]] |
...
| Product Capability Baseline | [[PCB-0001_PRODUCT_CAPABILITY_BASELINE|PCB-0001]] v2.7 (pointer-synced ESR-0050 WP7; content-refreshed WP1); sibling [[JARVIS_CAPABILITY_READINESS_MATRIX|JARVIS Capability Readiness Matrix]] v2.6 (same pattern) |
| Latest Repository Product Capability Assessment | [[RPCA-0001_REPOSITORY_PRODUCT_CAPABILITY_ASSESSMENT|RPCA-0001]] |
| Current Activity | See Section 3 (Current Mode) / Section 4A (Current Engineering Focus) - restructured at ESR-0031 (Documentation Debt Discipline) to reference the single authoritative source rather than restate it independently. Latest figures (ESR-0050 WP6): `python scripts/validate_repository.py` 0 errors, 289 warnings; `python -m pytest` 512 passed, 1 skipped; 13 Playwright tests passed. |
