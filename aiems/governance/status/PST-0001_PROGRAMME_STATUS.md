...
| Primary Branch | main |
| Current Mode | [[ESR-0026_ENGINEERING_SESSION_REPORT|ESR-0026]] remains the latest **closed** session (17 July 2026); per PBK-0001 WP0B, this field references the latest closed session until ESR-0027 itself closes and its report is authored. **ESR-0027 is open** (18 July 2026), run under the permanent [[EE-0001_INDEPENDENT_AI_PEER_REVIEW_TRIAL|EE-0001]] Section 7 appointment. **WP1 complete**: EBG-0080 (Personal Memory Implementation) implemented per [[EIP-ESR0027-001_PERSONAL_MEMORY_IMPLEMENTATION|EIP-ESR0027-001]] v1.1 - a working minimal consent gate for Personal Memory (MDS-0001 Section 6.2), the first Sentinel `REVIEW` decision anywhere in this codebase actually resolved by a human rather than only classified. Full real bridge review cycle across pre- and post-commit rounds: v0.1 drafted, Codex returned two Medium and one Low finding, v0.2 addressed all three and was confirmed, one further Low/editorial finding corrected at v0.3, Programme Sponsor approved via `sponsor-decision`, implemented and committed at v1.0. **Post-commit review of the real diff found two further Medium findings** (GuardianRuntime memory methods missing a runtime-state boundary check; PersonalMemoryStore.add() not enforcing durable consent traceability at the storage layer) - both fixed at v1.1, **Engineering Reviewer final confirmation: Pass, no new blocking findings, WP1 fully closed.** A genuine SQLite connection-leak defect was also found and fixed during the package's own required live smoke check. 286 tests total (was 254). **WP2 complete**: EBG-0077 (UXP static placeholder reconciliation) implemented per [[EIP-ESR0027-002_UXP_DIAGNOSTICS_PANEL_RECONCILIATION|EIP-ESR0027-002]] v1.0 - `first-light` removed from `DiagnosticsPanel`, `boundary`/`shell`/`agents` kept as honest, roadmap-grounded diagnostics. Engineering Reviewer post-implementation review: Pass, no blocking findings. Both of ESR-0027's Work Packages now complete, both fully closed with clean final confirmations. **Session-wide WP6** (Pass, two Low findings fixed) and **WP7** (Accept, [[RBL-0015_REPOSITORY_BASELINE|RBL-0015]] retained) both closed via the bridge, per [[ESR-0027_WP6_INDEPENDENT_REPOSITORY_VERIFICATION_HANDOVER|ESR-0027 WP6 handover]] v0.3. |
| Current Repository Baseline | [[RBL-0015_REPOSITORY_BASELINE|RBL-0015]] retained at ESR-0027 WP7 (Accept - Programme Sponsor determination, agreeing with both independent views: Engineering Reviewer and Engineering Implementer). Session-wide WP6 Independent Repository Verification also complete (Pass, two Low findings fixed). See [[ESR-0027_WP6_INDEPENDENT_REPOSITORY_VERIFICATION_HANDOVER|ESR-0027 WP6 handover]] v0.3. |
| Current Product Capability Baseline | [[PCB-0001_PRODUCT_CAPABILITY_BASELINE|PCB-0001]] v2.1, refreshed and accepted at ESR-0020 WP4 - not yet refreshed to reflect ESR-0027 WP1's Personal Memory implementation; still shows Memory as not implemented pending a future refresh pass. Its sibling document, [[JARVIS_CAPABILITY_READINESS_MATRIX|JARVIS Capability Readiness Matrix]], was similarly refreshed to v2.0 at ESR-0021 WP7 and is likewise not yet updated for this session. |
...
| Current Phase | ESR-0027 open since 18 July 2026 (report not yet authored - created at closure per established practice), under the permanent [[EE-0001_INDEPENDENT_AI_PEER_REVIEW_TRIAL|EE-0001]] Section 7 appointment (Claude Engineering Implementer, ChatGPT/Codex Engineering Reviewer, Programme Sponsor gating). |
| Current Workflow | AIEMS Engineering Workflow v3 with Engineering Ecosystem Synchronisation working practice. ESR-0027 WP1 continued the standing draft/review/approval/implementation/commit/post-commit-review template established at ESR-0026, entirely through `scripts/aiems_bridge.py`. |
| Current Engineering Objective | ESR-0027 WP1 (EBG-0080) and WP2 (EBG-0077) both Complete; session-wide WP6 (Pass) and WP7 (Accept, RBL-0015 retained) both closed. ESR-0027 remains open pending its own session report, authored at closure per established practice. |
...
ESR-0027 is **open** (18 July 2026; report not yet authored - created at closure per established practice), run under the permanent Lead/Reviewer appointment: Claude as Engineering Implementer, Codex (ChatGPT) as Engineering Reviewer, Programme Sponsor gating every step, continuing entirely through the AIEMS Exchange Bridge with no manual relay.

Programme Sponsor selected two Work Packages at WP0B: WP1 (Personal Memory implementation, the higher-priority selection) and WP2 (EBG-0077, UXP static placeholder reconciliation, queued behind WP1).

**WP1 (Complete)** implemented **EBG-0080** (Personal Memory Implementation) per [[EIP-ESR0027-001_PERSONAL_MEMORY_IMPLEMENTATION|EIP-ESR0027-001]] v1.0. A WP0-style scope check during drafting found GAM-0001's own evidence check accurate - no approval workflow existed in code for any Sentinel `REVIEW` decision, only a static message - and the Programme Sponsor chose to build the minimal mechanism rather than defer or descope. `jarvis/memory/store.py` (`PersonalMemoryStore`, SQLite-backed, `personal_memory` and a durable `consent_decisions` table) and `jarvis/memory/service.py` (`PersonalMemoryService.propose()/approve()/deny()`, reusing Sentinel's existing `requires_approval=True` classification path unmodified) wired into `GuardianRuntime`'s previously-dormant `"Guardian Memory Boundary"` and four new `stdio_rpc.py` JSON-RPC methods.

**Full real bridge review cycle**: v0.1 drafted and submitted to Codex, who returned two Medium findings (consent traceability under-specified - the original design had no durable record of a resolved decision; validation criterion inconsistent with the evidence actually submitted) and one Low finding (`propose()` should explicitly guard against non-`REVIEW` Sentinel outcomes). All three addressed at v0.2, which Codex confirmed resolved all three with no blocking findings, plus one further Low/editorial finding (a stale cross-reference to "logging output" after v0.2 added the durable table) corrected at v0.3. Programme Sponsor approved v0.3 via `sponsor-decision`.

**A genuine defect was found and fixed during WP1's own required live smoke check**: `PersonalMemoryStore`'s SQLite connections were never explicitly closed (`sqlite3.Connection`'s context manager only commits/rolls back, never closes) - harmless on most platforms but caused a real Windows `PermissionError` when the smoke check's temp directory tried to clean up. Fixed with an explicit `_transaction()` context manager that always closes the connection; the smoke check (propose->approve->list, propose->deny->list confirming a denied item never appears, and a fresh process confirming durable persistence across restart) then passed cleanly.

**A related pre-existing gap was also found and fixed**: every prior test in `jarvis/tests/test_stdio_rpc.py` that called `build_default_runtime()` had been omitting a memory-db override, meaning the full pre-ESR-0027 test suite would have created and written to the real `~/.jarvis/memory/personal.db` on whatever machine ran it once this package's code existed - the same test-isolation defect class ESR-0026 WP1 found for Ollama's network endpoint. Fixed retroactively across all affected tests, not just the new ones.

**Post-commit review of the real committed diff (505023c) found two further Medium blocking findings**, both fixed: (1) `GuardianRuntime`'s memory methods checked only service connectivity, not runtime state - a connected-but-not-started runtime could propose/approve/list memory, unlike `converse()`'s two-check pattern; fixed by also requiring `RUNNING` state, with two new regression tests (before-start, after-stop). (2) `PersonalMemoryStore.add()` accepted any `consent_decision_id` string without verifying a matching approved decision existed, so the storage layer itself - not just the one caller's careful ordering - could create orphan records; fixed by checking, inside the same transaction as the insert, that the referenced decision exists and is `"approved"`.

**Engineering Reviewer final confirmation (Codex, via the bridge, against fix commit dbd8424): Pass** - both Medium findings resolved, no new blocking findings. One non-blocking advisory recorded, not actioned: the store's traceability guard would permit multiple records referencing the same approved decision if a future caller bypassed `PersonalMemoryService` and called `PersonalMemoryStore.add()` directly - not a current gap, since the service always creates exactly one record per approval. **WP1 (EBG-0080) fully closed end-to-end.**

286 tests pass (was 254) and `validate_repository.py` (full mode) passes clean (120 warnings: 111 pre-existing plus 9 known, disclosed cross-document Section-reference false positives from this package's own governance artefacts).

**WP2 (Complete)** implemented **EBG-0077** (UXP Static Placeholder Row Reconciliation) per [[EIP-ESR0027-002_UXP_DIAGNOSTICS_PANEL_RECONCILIATION|EIP-ESR0027-002]] v0.2. Row-by-row reconciliation of `DiagnosticsPanel`'s four static rows against UAM-0001 Section 11 (Diagnostics Philosophy) and Section 14 (Colour Language): `boundary`/`shell`/`agents` kept unchanged (honest, UAM-0001-grounded statements, `agents` confirmed to map to a genuinely-not-started-but-roadmapped Agent Framework/GIA capability via `JARVIS_CAPABILITY_READINESS_MATRIX.md`); `first-light` removed (true but reporting on an unrelated legacy Tkinter product, not this shell's own boundary). Engineering Reviewer (Codex) confirmed the concept with no blocking findings and one Low/editorial finding (a stale comment enumerating the four rows), addressed at v0.2. Programme Sponsor approved via `sponsor-decision`. Implemented: `first-light` removed from `src/platformStatus.js` and `src/App.jsx`'s `diagnosticIcons`, the now-unused `Zap` import removed, the stale comment corrected. Verified live via an ad hoc Playwright check against the real Vite dev server (mocking the Tauri invoke bridge, no live backend, per the EBG-0072/EBG-0073 precedent): `DiagnosticsPanel` renders exactly three rows, zero console errors. `npm run build` clean. **Post-implementation review (Codex, via the bridge, against committed diff f29cf66): Pass, no blocking findings** - every claim independently verified, no fix cycle needed.

ESR-0027's Programme Sponsor-selected objective (Memory implementation + UXP placeholder reconciliation) is now fully delivered: both WP1 and WP2 complete, both with clean final Engineering Reviewer confirmations.

**Session-wide WP6** (Independent Repository Verification): **Pass** - Engineering Reviewer independently verified repository state via the bridge, found two Low findings (overstated cleanliness wording, one trailing-whitespace line), both fixed. **Session-wide WP7** (Repository Baseline Acceptance): **Accept**, [[RBL-0015_REPOSITORY_BASELINE|RBL-0015]] retained, both independent views converging (WP1's new backend capability not yet UXP-reachable, WP2's UXP change a small reviewed subtraction - neither resembling the kind of change that established RBL-0015 itself).

286 tests pass, `validate_repository.py` 0 errors throughout. Full detail in [[ESR-0027_WP6_INDEPENDENT_REPOSITORY_VERIFICATION_HANDOVER|ESR-0027 WP6 handover]].
...
## Prior Session: ESR-0025

[[ESR-0025_ENGINEERING_SESSION_REPORT|ESR-0025]] is **Closed** (17 July 2026). WP1 resolved the EBG-0060/EBG-0057 overlap and implemented EBG-0057 (Claude<->Codex Engineering Bridge) per [[EIP-ESR0025-001_AIEMS_EXCHANGE_BRIDGE_MVP|EIP-ESR0025-001]] v1.2, hardened across three post-implementation review rounds. Also produced a draft Ollama EIP (carried forward), a desktop launcher (EBG-0078), and two backlog registrations. Session-wide WP6 Pass, WP7 Accept, [[RBL-0015_REPOSITORY_BASELINE|RBL-0015]] retained. [[ESR-0025A_POST_CLOSURE_ENGINEERING_ADDENDUM|ESR-0025A]] (post-closure addendum) fixed the bridge's Windows preflight crash, a Programme Sponsor-authorised deviation, enabling ESR-0026's fully-real usage. Full detail in [[ESR-0025_ENGINEERING_SESSION_REPORT|ESR-0025]] and [[ESR-0025A_POST_CLOSURE_ENGINEERING_ADDENDUM|ESR-0025A]].

---

PST-0001 records the ESR-0026 closed state. It does not open a new engineering session or approve implementation outside separately authorised engineering work. Full ESR-0023 detail remains in [[ESR-0023_ENGINEERING_SESSION_REPORT|ESR-0023]] itself and [[REG-0001_CONTROLLED_ARTEFACT_REGISTER|REG-0001]]'s version history.
...
2. Review [[RBL-0015_REPOSITORY_BASELINE|RBL-0015]], the repository baseline the Programme Sponsor has accepted as current (retained at ESR-0026 WP7, not superseded).
3. Review [[PST-0001_PROGRAMME_STATUS|PST-0001]] (Current State tier).
4. Review [[JRM-0001_PROJECT_ROADMAP|JRM-0001]] (Project Roadmap, v1.13) for horizon-placement guidance before selecting the next Work Package - per PBK-0001's Backlog Progression Analysis, an item's existing JRM-0001 placement should inform the recommendation rather than being re-derived from scratch. **Track B (JARVIS/Guardian product): EBG-0019's architecture is now defined ([[MDS-0001_MEMORY_AND_DATA_STORAGE_ARCHITECTURE|MDS-0001]] v1.0)** - a real Memory implementation package building `jarvis/memory/` against it is the natural next Track B candidate, now that the specification exists (`jarvis/memory/` remains an empty stub). Other ready Track B candidates: EBG-0058, EBG-0065, EBG-0068, the REG-0001 HST/FCH gap, EBG-0046, EBG-0045/EBG-0049, EBG-0077 (UXP placeholder-row reconciliation). **Track A (AIEMS process): EBG-0057 (Claude<->Codex Engineering Bridge) is now proven in genuine real use across an entire session (ESR-0026)** - REA (its own future-phase scope) is a candidate once the MVP core has been proven in more real use. EBG-0075 (Ollama Local Fallback Provider) and EBG-0076 (network exposure security requirements, [[ADR-0020_SENTINEL_NETWORK_EXPOSURE_SECURITY_REQUIREMENTS|ADR-0020]] v1.0) both resolved at ESR-0026.
...
7. Review [[ESR-0026_ENGINEERING_SESSION_REPORT|ESR-0026]] (Current ESR tier, Closed) - the first session run entirely through `scripts/aiems_bridge.py` with no manual relay, now the standing template for future Work Packages (draft, review, approval, implementation, commit, post-commit review). Also review [[ESR-0023_ENGINEERING_SESSION_REPORT|ESR-0023]] Sections 9.1/10.1/10.2 as a live example of a repository write-boundary deviation, its root cause, and how it was found and fixed - directly relevant to the bridge's role-locked-permissions design.
...
This guidance records the ESR-0026 closed state and RBL-0015's continued retention (accepted at ESR-0026 WP7). PST-0001 does not create a new engineering session or approve implementation outside separately authorised engineering work.
