...
| Primary Branch | main |
| Current Mode | [[ESR-0024_ENGINEERING_SESSION_REPORT|ESR-0024]] is the latest **closed** session (17 July 2026). ESR-0025 is now **open** (no session report artefact yet - authored later per established practice); its WP1 resolved the EBG-0060/EBG-0057 overlap (DCE Superseded, REA folded into EBG-0057's future-phase scope, `fdb82f7`) and implemented EBG-0057 (Claude<->Codex Engineering Bridge) per [[EIP-ESR0025-001_AIEMS_EXCHANGE_BRIDGE_MVP|EIP-ESR0025-001]] v1.2 (v1.0 approved and implemented; two post-implementation Engineering Reviewer findings then addressed - see below): `scripts/aiems_bridge.py` implements all five commands (`init`, `submit-to-review`, `return-findings`, `sponsor-decision`, `submit-response`), with role-locking enforced as a real code gate rather than only a documented convention - `return-findings` has no code path capable of writing outside `.aiems-exchange/`, and `submit-response` hard-refuses unless the transcript's most recent `sponsor-decision` approves and the current repository HEAD matches the exact state that decision approved. **Post-implementation review of the diff found two further issues, both fixed:** High - `session`/`work_package` fed unsanitised into path construction could escape `.aiems-exchange/` via `..`/path-separator injection; fixed by validating both identifiers inside `_wp_key()`. Medium - `submit-to-review`/`submit-response` never checked `pytest`/`validate_repository.py` exit codes, so a failing run could produce a handover that looked successful; `submit-response` now hard-refuses on validation failure, `submit-to-review` stays non-blocking but its evidence opens with an unmissable `VALIDATION: PASSED`/`FAILED` marker. **A third post-implementation finding (Medium, TOCTOU)** found `submit-response`'s authorisation/drift checks, preflight and evidence capture all ran before the Work Package lock was acquired, only locking for the final write - a concurrent `sponsor-decision` could land in that window and the response would still proceed on the stale approval already read. Fixed by moving the entire sequence inside the lock, proven by a test that injects a concurrent `sponsor-decision` during evidence capture and confirms it is genuinely blocked. 25 new tests (237 total) prove all properties directly. A live dry run and smoke check confirmed the authorisation-refusal, approval-success, preflight-refusal and path-traversal-refusal paths for real; the full successful cycle needs the Programme Sponsor's own environment (neither `claude` nor `codex` is on the Engineering Implementer's sandboxed shell `PATH`) - disclosed, not asserted as tested. Run as a separate AIEMS/engineering-tooling workstream, outside JRM-0001's JARVIS/Guardian product tracks. |
| Current Repository Baseline | [[RBL-0015_REPOSITORY_BASELINE|RBL-0015]] retained at ESR-0024 WP7, unchanged so far this session pending ESR-0025's own WP7 baseline determination. |
...
| Current Phase | ESR-0025 open, under the permanent [[EE-0001_INDEPENDENT_AI_PEER_REVIEW_TRIAL|EE-0001]] Section 7 appointment (Claude Engineering Implementer, ChatGPT/Codex Engineering Reviewer, Programme Sponsor gating). |
| Current Workflow | AIEMS Engineering Workflow v3 with Engineering Ecosystem Synchronisation working practice. ESR-0025 WP1 is the first session to deliver a JRM-0001 Track A (AIEMS Process) item rather than Track B (JARVIS/Guardian product) - EBG-0057, run deliberately as a separate engineering-tooling workstream per the Programme Sponsor's post-ESR-0024 decision. |
| Current Engineering Objective | ESR-0025 WP1 complete (EBG-0057). [[JRM-0001_PROJECT_ROADMAP|JRM-0001]] Track B's Near-term horizon still identifies **EBG-0019** (Memory and Data Storage Architecture) as the top JARVIS/Guardian-track candidate, unchanged this session. No further WP is created by this status update - continuing or closing ESR-0025 is a separate action. |
...
ESR-0025 is **open** (17 July 2026; no session report artefact yet - authored later per established practice), run under the permanent Lead/Reviewer appointment: Claude as Engineering Implementer, Codex (ChatGPT) as Engineering Reviewer, Programme Sponsor gating every step.

**WP1** resolved the JRM-0001-flagged EBG-0060/EBG-0057 overlap (`fdb82f7`) and implemented **EBG-0057** (Claude<->Codex Engineering Bridge) per [[EIP-ESR0025-001_AIEMS_EXCHANGE_BRIDGE_MVP|EIP-ESR0025-001]] v1.2 - this session's first delivery from JRM-0001 Track A (AIEMS Process) rather than Track B, run deliberately as a separate engineering-tooling workstream. The overlap resolution: EBG-0060's DCE component (single-AI direct execution) marked Superseded, citing ESR-0023's write-boundary incidents as direct empirical evidence against it; EBG-0060's REA component folded into EBG-0057's own future-phase scope. `scripts/aiems_bridge.py` implements all five commands (`init`, `submit-to-review`, `return-findings`, `sponsor-decision`, `submit-response`). Engineering Reviewer (Codex) found one High and one Medium finding on the v0.1 draft - `sponsor-decision` had no canonical storage location, since the inbox/outbox model only covers Claude/Codex, resolved by making it transcript-only; and repository drift since sponsor approval was only warned about, resolved by having `submit-response` hard-refuse on any mismatch against the approving `sponsor-decision`'s recorded state - both addressed at v0.2 before approval. Role-locking is enforced as a real code gate: `return-findings` has no code path capable of writing outside `.aiems-exchange/` (no file-path argument exists to exploit), and `submit-response` refuses before any file write unless the transcript's most recent `sponsor-decision` approves and the current repository HEAD matches the exact state that decision approved. **A second Codex review of the implemented diff found two further issues, both fixed at v1.1:** High - `session`/`work_package` fed unsanitised into path construction could escape `.aiems-exchange/`, undermining the containment claim; fixed by validating both identifiers inside `_wp_key()`, the single point every path-building function goes through. Medium - a failing `pytest`/`validate_repository.py` run could still produce a handover indistinguishable from a successful one; `submit-response` now hard-refuses on validation failure, `submit-to-review` stays non-blocking but its evidence opens with an unmissable `VALIDATION: PASSED`/`FAILED` marker. **A third review found one further Medium finding (TOCTOU), fixed at v1.2:** `submit-response` read the transcript, checked drift, ran preflight and captured evidence all *before* acquiring the Work Package lock, only locking for the final write - a concurrent `sponsor-decision` could land in that window and the response would still proceed on the stale approval already read. Fixed by moving the entire read-check-evidence-write sequence inside the lock, matching every other command's existing lock-before-write discipline; proven by a test injecting a concurrent `sponsor-decision` during evidence capture and asserting it is genuinely blocked, not merely that the code compiles. 25 new tests (237 total) prove all properties directly. A live dry run and smoke check confirmed the authorisation-refusal, approval-success, preflight-refusal and path-traversal-refusal paths for real; the full successful cycle needs the Programme Sponsor's own environment (neither `claude` nor `codex` is on the Engineering Implementer's sandboxed shell `PATH`) - disclosed as an honest gap, not asserted as tested.

237 tests pass (was 212) and `validate_repository.py` (full mode) passes clean throughout.
...
PST-0001 records the ESR-0025 open state (WP1 complete). It does not create a new engineering session, close the current one, or approve implementation outside separately authorised engineering work. Full ESR-0023 detail remains in [[ESR-0023_ENGINEERING_SESSION_REPORT|ESR-0023]] itself and [[REG-0001_CONTROLLED_ARTEFACT_REGISTER|REG-0001]]'s version history.
...
| Item | Status | Notes |
|------|--------|-------|
| Current Engineering Session | ESR-0025 - **Open** (17 July 2026), no session report artefact yet | WP1 complete: EBG-0060/EBG-0057 overlap resolved, EBG-0057 (Claude<->Codex Engineering Bridge) implemented and hardened per [[EIP-ESR0025-001_AIEMS_EXCHANGE_BRIDGE_MVP|EIP-ESR0025-001]] v1.2 - `scripts/aiems_bridge.py`, role-locking enforced as a real code gate. |
| Current Repository Baseline | [[RBL-0015_REPOSITORY_BASELINE|RBL-0015]] | Retained at ESR-0024 WP7, unchanged so far this session pending ESR-0025's own WP7 baseline determination. |
...
| Current Review State | ESR-0025 WP1 reviewed (Engineering Reviewer/Codex approved EIP-ESR0025-001 v0.2) | Session continues; not yet subject to session-wide WP6. |
| Next Required Activity | Continue or close ESR-0025 | Under the permanent Lead/Reviewer appointment. Not decided by this status update. |
| Next Work Package Candidate | [[JRM-0001_PROJECT_ROADMAP|JRM-0001]] Track B's Near-term horizon still leads with EBG-0019 (Memory and Data Storage Architecture) for JARVIS/Guardian-track work. EBG-0057's own REA future-phase scope (execution-automation agent) is a Track A candidate once the MVP core has been proven in real use. | Not selected by this status update. |
| ESR-0025 Delivered Scope (WP1) | EBG-0060/EBG-0057 overlap resolved (DCE Superseded, REA folded into EBG-0057). EBG-0057 MVP implemented: `scripts/aiems_bridge.py`, five commands, role-locking as a real code gate, 15 new tests. | Complete and reviewed. |
...
7. Review [[ESR-0024_ENGINEERING_SESSION_REPORT|ESR-0024]] (Current ESR tier - ESR-0025 has no session report artefact of its own yet, since it remains open; its WP1 is recorded in EBR-0001/JRM-0001/PST-0001 and [[EIP-ESR0025-001_AIEMS_EXCHANGE_BRIDGE_MVP|EIP-ESR0025-001]] in the meantime), and [[ESR-0023_ENGINEERING_SESSION_REPORT|ESR-0023]] Sections 9.1/10.1/10.2 as a live example of a repository write-boundary deviation, its root cause, and how it was found and fixed - directly relevant to `scripts/aiems_bridge.py` (EBG-0057), whose role-locked-permissions design exists specifically to prevent this class of failure by construction.
...
This guidance records the ESR-0025 open state (WP1 complete) and RBL-0015's continued (not yet re-determined) retention. PST-0001 does not create a new engineering session, close the current one, accept a repository baseline or approve implementation outside separately authorised engineering work.
...
| ESR-0025 | Current open engineering session (17 July 2026; no report artefact yet) - WP1 resolved the EBG-0060/EBG-0057 overlap and implemented EBG-0057 (Claude<->Codex Engineering Bridge) per [[EIP-ESR0025-001_AIEMS_EXCHANGE_BRIDGE_MVP|EIP-ESR0025-001]] v1.2. |
