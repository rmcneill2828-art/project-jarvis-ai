...
| Primary Branch | main |
| Current Mode | [[ESR-0044_ENGINEERING_SESSION_REPORT|ESR-0044]] is the latest **closed** session (30 July 2026). No session is currently open. **WP1** closed [[EBR-0001_ENGINEERING_BACKLOG_REGISTER|EBR-0001]] EBG-0114: Guardian's Voice faculty (EBG-0112, ESR-0040) is now reachable through the live running Tauri UXP for the first time, via [[EIP-ESR0044-001_WIRE_VOICE_INTO_LIVE_RUNTIME_AND_UXP|EIP-ESR0044-001]] (Codex design-reviewed, Pass with non-blocking findings; Programme Sponsor approval verified via the real Sponsor Approval Service). A 3-layer wiring change - `build_default_runtime()` conditionally constructs a Sentinel-gated Piper speech provider when `JARVIS_PIPER_VOICE_PATH` is set (mirroring the existing credential-gated pattern, no auto-download), a new `guardian.speak` RPC method and `speak_message` Tauri command bridge it, and a minimal per-message speak button in the UXP plays the audio - with no change to any already-approved Voice component itself. Live-verified end to end with a real 161,836-byte `audio/wav` payload. Session-wide **WP2** (Independent Repository Verification: Pass, no blocking findings) and **WP3** (Repository Baseline Determination: **Establish** - [[RBL-0027_REPOSITORY_BASELINE|RBL-0027]] accepted, superseding RBL-0026, since this session delivered a genuine, live product-capability change) closed the session. Full detail in [[ESR-0044_ENGINEERING_SESSION_REPORT|ESR-0044]]. |
...
| Current Phase | ESR-0044 closed (30 July 2026); no session currently open; under the permanent [[EE-0001_INDEPENDENT_AI_PEER_REVIEW_TRIAL|EE-0001]] Section 7 appointment (Claude Engineering Implementer, ChatGPT/Codex Engineering Reviewer, Programme Sponsor gating). |
| Current Workflow | AIEMS Engineering Workflow v3 with Engineering Ecosystem Synchronisation working practice. ESR-0044 completed the standing draft/review/approval/implementation/commit/post-commit-review template entirely through direct `codex exec -s read-only` invocation and the deployed Sponsor Approval Service - confirmed repeatable across a nineteenth consecutive session (ESR-0026 through ESR-0044). No session is currently open; the next session will select its own objective at WP0B. |
| Current Engineering Objective | None - no session currently open. ESR-0044's WP1 plus session-wide WP2/WP3 are all Complete; see [[ESR-0044_ENGINEERING_SESSION_REPORT|ESR-0044]] for full detail. |
...
[[ESR-0044_ENGINEERING_SESSION_REPORT|ESR-0044]] is **closed** (30 July 2026). **No session is currently open.** The nineteenth session run entirely through direct Claude<->Codex CLI invocation with no manual relay of the review cycle itself, run under the permanent Lead/Reviewer appointment: Claude as Engineering Implementer, Codex as Engineering Reviewer, Programme Sponsor gating every step.

**WP1** (Complete): closed [[EBR-0001_ENGINEERING_BACKLOG_REGISTER|EBR-0001]] EBG-0114 - `build_default_runtime()` (the one construction path the real running Tauri UXP uses) never wired a speech provider, so Guardian's Voice faculty (EBG-0112, ESR-0040) was reachable only via standalone, uncommitted scripts. Fixed via [[EIP-ESR0044-001_WIRE_VOICE_INTO_LIVE_RUNTIME_AND_UXP|EIP-ESR0044-001]], a 3-layer change: `build_default_runtime()` conditionally constructs a Sentinel-gated Piper speech provider when `JARVIS_PIPER_VOICE_PATH` is present (mirroring the existing credential-gated real-provider pattern - absent means honest `not_connected`, no auto-download), reusing the existing single `SentinelTrustGateway`; a new `guardian.speak` JSON-RPC method serializes the outcome; a new `speak_message` Tauri command mirrors `send_message` exactly; a minimal per-message speak button in the UXP plays the audio, with non-blocking inline error notes for both backend and frontend failure modes. **Codex design-reviewed** via direct `codex exec -s read-only` invocation: Pass, with non-blocking findings, folded in. Programme Sponsor approval verified via the real Sponsor Approval Service before implementation began. No change to any already-approved Voice component itself (`PiperProvider`, `SentinelGatedSpeechProvider`, `GuardianRuntime.speak()`). **Live-verified end to end**: a real Piper model downloaded via `piper-tts`'s own `download_voices` CLI, a genuine `guardian.speak` call through the exact new wiring produced a real 161,836-byte `audio/wav` payload. Full detail in [[ESR-0044_ENGINEERING_SESSION_REPORT|ESR-0044]].

Session-wide **WP2** (Independent Repository Verification): **Pass, no blocking findings** - Codex independently reviewed the pushed commit `899e67f`, confirmed diff scope matched approvals (exactly 11 files, none outside scope), confirmed via direct `git diff` that every already-approved Voice component was genuinely untouched, and confirmed the RPC/Tauri wiring shapes were correct. One non-blocking observation (whitespace-only `JARVIS_PIPER_VOICE_PATH` treated as configured, consistent with existing codebase convention) was disclosed, not treated as blocking. Codex's own sandbox hit the same disclosed spawn-error limitation running `validate_repository.py`/`pytest` directly (EBG-0096 precedent); the Engineering Implementer independently re-ran both against the real pushed HEAD to complete the evidence. Session-wide **WP3** (Repository Baseline Determination): **Establish** - [[RBL-0027_REPOSITORY_BASELINE|RBL-0027]] accepted as the new current repository baseline, superseding [[RBL-0026_REPOSITORY_BASELINE|RBL-0026]], since this session delivered a genuine, live product-capability change.

424 Python tests pass plus 1 correctly-skipped test (up from 418/1, 6 new); Rust build/clippy/fmt/test all clean; Playwright suite (7, 2 new) passes; `validate_repository.py` (full mode) passes clean (258 warnings, unchanged from session start).

---

## Prior Session: ESR-0043

[[ESR-0043_ENGINEERING_SESSION_REPORT|ESR-0043]] is **Closed** (30 July 2026). The eighteenth session run entirely through direct Claude<->Codex CLI invocation with no manual relay of the review cycle itself. **WP1** refined Guardian's persona toward the classic film J.A.R.V.I.S. characterisation via EIP-ESR0043-001, additive-only, no existing approved text reworded or removed. Session-wide WP2 Pass (no findings), WP3 Establish - [[RBL-0026_REPOSITORY_BASELINE|RBL-0026]] accepted, superseding RBL-0025 (real persona behaviour change). Full detail in [[ESR-0043_ENGINEERING_SESSION_REPORT|ESR-0043]].

---

PST-0001 records ESR-0044 as the latest closed session and no session currently open. It does not itself approve implementation outside separately authorised engineering work. Full historical detail remains in each session's own report and [[REG-0001_CONTROLLED_ARTEFACT_REGISTER|REG-0001]]'s version history.
...
| JARVIS Product Capability Baseline | Accepted, Current | Foundation | [[PCB-0001_PRODUCT_CAPABILITY_BASELINE|PCB-0001]] v2.2 - refreshed at ESR-0031 WP0 to reflect ESR-0027 WP1's Personal Memory implementation. No longer stale. |
...
| Current Product Baseline | [[PCB-0001_PRODUCT_CAPABILITY_BASELINE|PCB-0001]] | v2.2 - refreshed at ESR-0031 WP0 to reflect ESR-0027 WP1's Personal Memory implementation. Sibling [[JARVIS_CAPABILITY_READINESS_MATRIX|JARVIS Capability Readiness Matrix]] refreshed to v2.2 at ESR-0028 WP2/WP4. Memory's maturity is now Partial (Personal tier implemented, Session/Shared-Family tiers not yet built) - no longer Not Started. |
