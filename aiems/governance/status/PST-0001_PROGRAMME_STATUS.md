...
| Primary Branch | main |
| Current Mode | [[ESR-0028_ENGINEERING_SESSION_REPORT|ESR-0028]] is the latest **closed** session (18 July 2026). No further session has yet been opened. ESR-0028 was the third session run entirely through the AIEMS Exchange Bridge (`scripts/aiems_bridge.py`) with no manual relay anywhere, across five Work Packages: WP1 (EBG-0058/0005/0068/0071 process hygiene batch), WP2 (EBG-0017 product requirements backlog identification), WP3 (EBG-0045/0049/0024 cost and strategic value framework), WP4 (EBG-0055 Phase 1.5 Guardian Orb 3D rotation, seven live-verified fix rounds after a real Windows Task Manager power-usage finding), and WP5 (ADR-0021, Guardian Orb rendering engine decision - Canvas 2D over continued SVG or WebGL, decision only, no implementation). Session-wide WP6 (Pass, no findings) and WP7 (Accept - new baseline [[RBL-0016_REPOSITORY_BASELINE|RBL-0016]] established, diverging from the prior two sessions' retain-RBL-0015 pattern) both closed via the bridge. 286 tests total (unchanged this session). Full detail in [[ESR-0028_ENGINEERING_SESSION_REPORT|ESR-0028]]. |
...
| Current Phase | No engineering session currently open; ESR-0028 closed 18 July 2026, under the permanent [[EE-0001_INDEPENDENT_AI_PEER_REVIEW_TRIAL|EE-0001]] Section 7 appointment (Claude Engineering Implementer, ChatGPT/Codex Engineering Reviewer, Programme Sponsor gating). |
...
[[ESR-0028_ENGINEERING_SESSION_REPORT|ESR-0028]] is **Closed** (18 July 2026), run under the permanent Lead/Reviewer appointment: Claude as Engineering Implementer, Codex (ChatGPT) as Engineering Reviewer, Programme Sponsor gating every step. **The third session run entirely through the AIEMS Exchange Bridge with no manual relay anywhere.**

**WP1** closed **EBG-0058/EBG-0005/EBG-0068/EBG-0071** (AIEMS process hygiene batch) per [[EIP-ESR0028-001_AIEMS_PROCESS_HYGIENE_BATCH|EIP-ESR0028-001]] v1.0 - registered 10 previously-untracked HST/FCH artefacts, consolidated a three-times-restated repository-operations-authorisation clause in PBK-0001 into cross-references, closed two long-dormant backlog items (Resolved by Attrition, Superseded) with disclosed rationale.

**WP2** closed **EBG-0017** (JARVIS product requirements backlog identification) per [[EIP-ESR0028-002_JARVIS_PRODUCT_REQUIREMENTS_BACKLOG_IDENTIFICATION|EIP-ESR0028-002]] v1.0 - identified (not created) `JARVIS_PRODUCT_ARCHITECTURE.md` and the Capability Readiness Matrix as the already-existing authoritative product requirements backlog, registered both in REG-0001 for the first time, refreshed the Matrix to v2.1/v2.2 correcting two genuinely stale rows (Memory, Provider Architecture) against direct evidence.

**WP3** closed **EBG-0045/EBG-0049/EBG-0024** (cost and strategic value framework) per [[EIP-ESR0028-003_COST_AND_STRATEGIC_VALUE_FRAMEWORK|EIP-ESR0028-003]] v1.0 - revised [[PEM-001_AI_PROVIDER_EVALUATION_MATRIX|PEM-001]] (not a new document) to make Cost a genuine first-class criterion (10% to 15%) with an explicit no-discretionary-budget default, and recorded a cost-aware routing policy statement while explicitly excluding any dynamic routing implementation as premature.

**WP4** delivered **EBG-0055 Phase 1.5** (Guardian Orb 3D rotation) per [[EIP-ESR0028-004_GUARDIAN_ORB_3D_ROTATION|EIP-ESR0028-004]] v1.0 - full-sphere coordinates with real Y-axis rotation, `prefers-reduced-motion` support, and a directionally-informed, empirically-reverified force retuning. The Programme Sponsor found a real power-usage regression via Windows Task Manager after initial delivery, requiring **seven further live-verified fix rounds**: a ref-based `requestAnimationFrame` rewrite (replacing React-state-driven rendering), removal of an unrelated pre-existing box-shadow animation, a capped update rate, decoupled node/edge update frequencies, an idle-timeout pause, and a `visibilitychange`-based fix for a hidden-window resume race - each verified via Playwright against the real 195-node/1,687-edge graph, the last two confirmed by Codex with no further findings.

**WP5** approved **[[ADR-0021_GUARDIAN_ORB_RENDERING_ENGINE|ADR-0021]]** (decision only, no implementation) - migrate `GuardianOrbGraph.jsx` from SVG to Canvas 2D when scoped, grounded directly in WP4's seven-round evidence; WebGL/Three.js explicitly rejected as disproportionate to current/anticipated scale. Applied a split disposition to **EBG-0081**: the rendering-engine half Complete via this ADR, the shared-animation-scheduler half remains open Candidate Backlog.

Session-wide **WP6** (Independent Repository Verification): **Pass**, no findings. Session-wide **WP7** (Repository Baseline Acceptance): **Accept**, new baseline [[RBL-0016_REPOSITORY_BASELINE|RBL-0016]] established, superseding RBL-0015 - both independent views converging that WP4's Guardian Orb change is a materially new user-visible product capability/experience change, diverging from the prior two sessions' retain-RBL-0015 pattern.

286 tests pass (unchanged - WP1/2/3/5 were governance-only, WP4 touched only `src/`) and `validate_repository.py` (full mode) passes clean throughout (126 warnings baseline). Full detail in [[ESR-0028_ENGINEERING_SESSION_REPORT|ESR-0028]].
...
7. Review [[ESR-0028_ENGINEERING_SESSION_REPORT|ESR-0028]] (Current ESR tier, Closed) - the third session run entirely through `scripts/aiems_bridge.py` with no manual relay, confirming the draft/review/approval/implementation/commit/post-commit-review template is genuinely repeatable across a third session, and the first session to establish a new repository baseline (RBL-0016) since ESR-0022. Also review [[ESR-0023_ENGINEERING_SESSION_REPORT|ESR-0023]] Sections 9.1/10.1/10.2 as a live example of a repository write-boundary deviation, its root cause, and how it was found and fixed - directly relevant to the bridge's role-locked-permissions design.
...
This guidance records the ESR-0028 closed state and RBL-0016's acceptance at ESR-0028 WP7 (superseding RBL-0015). PST-0001 does not create a new engineering session or approve implementation outside separately authorised engineering work.
