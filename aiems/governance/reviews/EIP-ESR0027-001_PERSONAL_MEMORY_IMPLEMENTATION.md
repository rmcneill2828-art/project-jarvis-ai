...
|---------|------|--------|---------|
| 1.1 | 18 July 2026 | Claude Engineering Implementer | Post-commit review of the real committed diff (505023c) by the Engineering Reviewer via the bridge found two Medium blocking findings, both fixed. Finding 1: `GuardianRuntime`'s memory methods checked only service connectivity, not runtime state - a connected-but-not-started runtime could propose/approve/list memory, unlike `converse()`'s two-check pattern. Fixed: `_require_memory_service()` now also requires `RUNNING` state; two new regression tests (before-start, after-stop). Finding 2: `PersonalMemoryStore.add()` accepted any `consent_decision_id` string without verifying a matching approved decision actually existed, letting the storage layer itself create orphan records - the durable-traceability guarantee only held because the one caller (`PersonalMemoryService.approve()`) happened to call things in the right order, not because the store enforced it. Fixed: `add()` now checks, inside the same transaction as the insert, that `consent_decision_id` references a recorded `"approved"` decision, raising `ValueError` otherwise; new tests cover the no-decision and denied-decision rejection cases. Engineering Reviewer confirmed the original `_transaction()` leak fix was sound and the memory-db test-isolation fix was complete across every call site. 286 tests total (was 282). |
