...
|---------|------|--------|---------|
| 1.3 | 17 July 2026 | Claude Engineering Implementer | **Post-closure fix, Programme Sponsor-authorised deviation from the standard Working Report Lifecycle** (no EIP drafted or Engineering Reviewer review before implementation - recorded transparently at [[ESR-0025A_POST_CLOSURE_ENGINEERING_ADDENDUM|ESR-0025A]]). The Programme Sponsor asked whether v1.2's disclosed gap ("neither `claude` nor `codex` is on the Engineering Implementer's sandboxed shell `PATH`") meant the bridge could never work; both CLIs were installed and authenticated live to test this, surfacing a genuine second defect: `run_preflight()`'s `subprocess.run([tool, "--version"], ...)` crashed with an unhandled `FileNotFoundError` (WinError 2) on Windows, because npm installs global CLI tools as `.CMD` shim files that Windows' process-creation API cannot launch without `shell=True` - a bug invisible until both tools actually existed on a Windows PATH, which nobody had done before now. Fixed: `shell=(sys.platform == "win32")` added to both `subprocess.run` calls in `run_preflight()`; also corrected `["codex", "login-status"]` (wrong, single hyphenated argument) to `["codex", "login", "status"]` (the real subcommand). One new test (`test_run_preflight_invokes_subprocess_with_shell_true_on_windows`) exercises the real function via a reference captured before the test file's autouse preflight stub patches it - 238 tests total (was 237). Live-verified end-to-end: all five commands (`init`, `submit-to-review`, `return-findings`, `sponsor-decision`, `submit-response`) run cleanly against the real, now-authenticated `codex`/`claude` CLIs, including `submit-response`'s full authorisation/drift/validation gate sequence. Does not itself prove Codex will autonomously act on its turn without prompting - only that the commands no longer crash. |
