...
**Relevance to EE-0001 scoring:** this bears on Section 5.6 (Reviewer role discipline - not applicable here, Lead role) more precisely on Lead scope/process discipline and Section 5.11 (Sponsor arbitration required) - three rounds of correction before empirical verification occurred is a real arbitration cost, to be weighed at ESR-0016 closure alongside whatever WP1/WP2 substantive findings accumulate. Consistent with [[EE-0001_INDEPENDENT_AI_PEER_REVIEW_TRIAL|EE-0001]] Section 2's warning against pre-loading conclusions, this is recorded as observed fact for scoring, not adjusted for or excused on tooling grounds - and, symmetrically, not exaggerated past what the ESR-0015 counter-evidence above actually supports.
...
|---|---|---|---|
| Findings raised / accepted / rejected / false positive | ~12 / 12 / 0 / 0 (draft estimate) | 13 / 11 / 0 / 0, 2 since resolved | **7 / 7 / 0 / 0** - resolved per EE-0001 Section 8's dated definition (submitted-work defects only): WP1 x4 (precedence, exports, tests, collection error), WP2 x3 (unapproved artefact, Guidance Note proposal, version-bump gap). Pre-implementation EIP refinements and behavioural incidents excluded from this count, scored instead under Lead scope discipline / Sponsor arbitration below |
| Average defect discovery weight | ~2.8 (draft estimate) | 3.0 (initial), revised to ~2.88 on reattribution | **3.0** - under the narrowed definition, all 7 findings are Reviewer-caught (weight 3 each); the Sponsor-caught and Lead-self-caught behavioural incidents fall outside the findings count entirely under this definition, so the earlier ~2.8/2.88 reattribution no longer applies to this metric specifically |
...
**Findings count - resolved by Programme Sponsor decision, logged in EE-0001 Section 8.** The Lead's ~12 and the Reviewer's 13 differed because neither draft defined "findings" (EE-0001 Section 5.1) precisely. The Programme Sponsor decided the narrow definition: findings are limited to defects in submitted/implemented work, excluding pre-implementation EIP refinements and behavioural incidents (the latter remain scored under Lead scope discipline and Sponsor arbitration, not double-counted here). Confirmed this matches how ESR-0015's count of 10 was already composed, so ESR-0015 required no retroactive correction. ESR-0016 recomputes to **7 findings, average discovery weight 3.0**, both under the corrected definition. Dated entry recorded in [[EE-0001_INDEPENDENT_AI_PEER_REVIEW_TRIAL|EE-0001]] Section 8.
