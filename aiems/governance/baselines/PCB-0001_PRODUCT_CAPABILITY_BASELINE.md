...
- [[RBL-0027_REPOSITORY_BASELINE]] (current accepted repository baseline, established at ESR-0044; superseded RBL-0007 through RBL-0026 since PCB-0001 v1.0)
...
| Voice faculty | Guardian's Voice faculty, Phase 6 Increment A (speech output only), is implemented via a self-hosted Piper local-TTS provider (`sentinel/piper_provider.py`, credential/path-gated, no API key or recurring cost - ESR-0040) and is now wired into the live JSON-RPC bridge (`guardian.speak`) and the Guardian Desktop Platform Shell's speak button (ESR-0044, EIP-ESR0044-001), live-verified with a real synthesized audio payload. Speech input remains not implemented (see Section 6). |
...
- Voice capability is implemented for speech output only (Phase 6 Increment A, self-hosted Piper, ESR-0040), wired into the live UXP (ESR-0044); speech input is not implemented.
...
- Local agent capability is not implemented. The Local Agent Permission Boundary is defined ([[GAM-0001_GUARDIAN_AUTHORITY_AND_BOUNDARY_MODEL|GAM-0001]] Section 8A, ESR-0041) - the policy boundary a future Local Agent/Action faculty implementation must obey - but no implementation exists yet.
